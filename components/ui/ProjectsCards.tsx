...
const ProjectsCards = ({
  ...
  function onCtaClick(e: React.MouseEvent | React.TouchEvent <HTMLButtonElement>) {
    e.stopPropagation();
    if (buttonLink) {
    ...
    <div className=" overflow-hidden border border-gray-600/40 shadow-2xl shadow-indigo-500/30 rounded-xl">
      <div className="w-full" style={{ perspective: "1000px" }}>
        <motion.div
          ref={cardRef}
          onClick={onCtaClick}
          className="group relative   w-[100%] aspect-video h-fit overflow-hidden 
        ...
        >
          <Image
            ...
            src={current.image}
            alt={current.name}
            className="absolute inset-0  w-full h-full object-cover opacity-95  transition-transform duration-700 ease-[cubic-bezier(0.33,1,0.68,1)] group-hover:scale-105 "
          />
          ...
          {/* Swipe indicator - animated modern arrow */}
          <div
            aria-hidden
            className="absolute top-1/2 -translate-y-1/2 right-[20px] z-40 flex h-[48px] w-[48px] items-center justify-center rounded-full bg-white/10 backdrop-blur-sm opacity-0 transition-opacity duration-200 group-hover:opacity-100"
          >
            <motion.svg
            ...
            {/* subtle glow ring behind the arrow */}
            <motion.span
              aria-hidden
              className="absolute inset-0 rounded-full"
              style={{ boxShadow: "0 12px 48px rgba(99,102,241,0.22)" }}
              animate={{
                opacity: [0.35, 0.95, 0.35],
                boxShadow: [
                  "0 8px 24px rgba(99,102,241,0.14)",
                  "0 18px 64px rgba(99,102,241,0.32)",
                  "0 8px 24px rgba(99,102,241,0.14)",
                ],
              }}
              transition={{
